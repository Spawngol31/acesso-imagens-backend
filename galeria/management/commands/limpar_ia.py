import boto3
import time
from django.core.management.base import BaseCommand
from django.utils import timezone
from datetime import timedelta
from django.conf import settings
from galeria.models import Foto, FaceIndexada

class Command(BaseCommand):
    help = 'Exclui faces da AWS Rekognition de fotos mais antigas que 60 dias'

    def handle(self, *args, **kwargs):
        # 1. Determina a linha de corte: 60 dias atrás
        data_limite = timezone.now() - timedelta(days=60)
        
        # 2. Encontra fotos antigas e pega os IDs das faces
        fotos_antigas = Foto.objects.filter(data_upload__lt=data_limite)
        faces_para_apagar = FaceIndexada.objects.filter(foto__in=fotos_antigas).values_list('rekognition_face_id', flat=True)
        
        lista_rostos = list(faces_para_apagar)
        
        if not lista_rostos:
            self.stdout.write(self.style.SUCCESS("✨ O cofre da IA está otimizado. Nenhum rosto antigo encontrado."))
            return

        client = boto3.client('rekognition', region_name=settings.AWS_REKOGNITION_REGION_NAME)
        
        # 🚀 MÁGICA 1: Lotes pequenos de 250 rostos (A AWS processa instantaneamente sem engasgar)
        lotes = [lista_rostos[i:i + 250] for i in range(0, len(lista_rostos), 250)]
        
        total_apagados = 0
        self.stdout.write(self.style.WARNING(f"Iniciando faxina de {len(lista_rostos)} rostos em {len(lotes)} lotes..."))

        for index, lote in enumerate(lotes):
            sucesso = False
            tentativas = 0
            
            # 🚀 MÁGICA 2: Sistema de Retentativa Inteligente
            while not sucesso and tentativas < 3:
                try:
                    response = client.delete_faces(
                        CollectionId=settings.AWS_REKOGNITION_COLLECTION_ID,
                        FaceIds=lote
                    )
                    apagados_no_lote = len(response.get('DeletedFaces', []))
                    total_apagados += apagados_no_lote
                    
                    # Remove do banco de dados local
                    FaceIndexada.objects.filter(rekognition_face_id__in=lote).delete()
                    
                    self.stdout.write(self.style.SUCCESS(f"Lote {index + 1}/{len(lotes)} limpo! ({apagados_no_lote} rostos)"))
                    sucesso = True
                    
                    # Descanso levíssimo de 1 segundo entre lotes pequenos
                    time.sleep(1)
                    
                except Exception as e:
                    tentativas += 1
                    if "ProvisionedThroughputExceededException" in str(e):
                        self.stdout.write(self.style.WARNING(f"⚠️ A AWS pediu para ir devagar. Pausando 10s... (Tentativa {tentativas}/3)"))
                        time.sleep(10) # Pausa dramática para a AWS respirar
                    else:
                        self.stdout.write(self.style.ERROR(f"Erro desconhecido: {str(e)}"))
                        break # Se for outro erro grave, aborta este lote
            
        self.stdout.write(self.style.SUCCESS(f"🎉 FAXINA CONCLUÍDA! {total_apagados} rostos antigos foram excluídos permanentemente da AWS."))